...
- When you run ``python3 -m quickstatements_client`` python will execute``__main__.py``
  as a script. That means there won't be any ``quickstatements_client.__main__`` in
  ``sys.modules``.
- When you import __main__ it will get executed again (as a module) because there's no
  ``quickstatements_client.__main__`` in ``sys.modules``.
